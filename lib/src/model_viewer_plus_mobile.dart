...
class ModelViewerState extends State<ModelViewer> {
  ...
  Future<void> _initProxy() async {
    ...
    _proxy = await HttpServer.bind(
      InternetAddress.loopbackIPv4,
      3000,
    );
    print('_proxy-----: ${_proxy}'); // DEBUG
    setState(() {
      _proxy;
      ...
      switch (request.uri.path) {
        ...
        case '/model':
          if (url.isAbsolute && !url.isScheme("file")) {
            ...
            response
              ..headers.add('Origin', '${_proxyURL}')
              ..headers.add('Access-Control-Allow-Origin', '${_proxyURL}');
            await response.redirect(
              url,
            ); // TODO: proxy the resource
  ...
  Future<Uint8List> _readNetWork(final String key) async {
    final uri = Uri.parse(key);
    final bundle = await NetworkAssetBundle(uri);
    final data = await bundle.load(uri.pathSegments.last);
    return data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes);
  }
